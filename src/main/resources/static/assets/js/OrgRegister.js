...
function validateData() {
	...
	if( usernameRes != true) {
		...
		containsError = containsError+"error ";
	} else {
	...
	if(!$("#acceptTerms").is(":checked")) {
		containsError = containsError+"error ";	
		$("#acceptTerms").css("border-color", "red");
		$(".invalid-terms").text("read and accept the terms and conditions");
	} else {
		$("#acceptTerms").css("border-color", "#ced4da");
		$(".invalid-terms").text("");
	}
	
	if(containsError.includes("error")) {
		return false;
	} else {
		return true;
	}
...
$(document).ready(function(){
	...
	/**
	 * on change event for username
	 */
	$("#inputUsername").on("change", function(e){
		let usernameRes = validateUsername();
			if( usernameRes != true) {
			$("#inputUsername").css("border-color", "red");
			$(".invalid_username").text(usernameRes);
	...
	$("#inputPasscode").on("change", function(e){
		let passCodeRes = validatePassword();
		if( passCodeRes != true) {
			$("#inputPasscode").css("border-color", "red");
			$(".invalid_password").text(passCodeRes);
	...
	$("#inputConfPasscode").on("change", function(e){
		let confPassCodeRes = validateConfirmPassword();
		if( confPassCodeRes != true) {
			$("#inputConfPasscode").css("border-color", "red");
			$("#inputPasscode").css("border-color", "red");
